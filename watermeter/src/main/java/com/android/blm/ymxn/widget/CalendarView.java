...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
        ...
        for (int i = 0; i < 42; i++) {
            ...
            if (todayIndex != -1 && i == todayIndex) {
                ...
                if (todayIndex != selectText[0] && todayIndex != selectText[1]) {
                    drawEndStart(canvas, todayIndex, "今天", surface.todayNumberColor);
                }
            }

            if (i == selectText[0] || i == selectText[1]) {
                color = surface.selectTextColor;
            }
            if (selectText[0] == selectText[1]) {
                drawEndStart(canvas, selectText[0], "开始", surface.todayNumberColor);
            } else {


                if (!(selectText[0] < 0) && !(selectText[1] < 0)) {

                    if (selectText[0] > selectText[1]) {

                    } else {
                        drawEndStart(canvas, selectText[1], "结束", surface.todayNumberColor);
                        drawEndStart(canvas, selectText[0], "开始", surface.todayNumberColor);
                    }
                    ...
                    if (selectText[1] > 0) {
                        drawEndStart(canvas, selectText[1], "开始", surface.todayNumberColor);
                    }
     ...
     * @param color
     */
    private void drawEndStart(Canvas canvas, int index, String text, int color) {
        int x = getXByIndex(index);
        int y = getYByIndex(index);
        surface.startEndPaint.setColor(color);
        float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
                * surface.cellHeight + surface.cellHeight * 9 / 10f;
        float cellX = (surface.cellWidth * (x - 1))
                + (surface.cellWidth - surface.datePaint.measureText(text));
        canvas.drawText(text, cellX, cellY, surface.startEndPaint);

        drawCellTextWithFlat(canvas, index, date[index] + "", color);
    }
     ...
     * @param text
     */
    private void drawCellTextWithFlat(Canvas canvas, int index, String text, int color) {
        int x = getXByIndex(index);
        int y = getYByIndex(index);
        surface.datePaint.setColor(surface.selectTextColor);
        float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
